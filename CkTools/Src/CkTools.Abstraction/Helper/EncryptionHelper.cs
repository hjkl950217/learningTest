...
namespace CkTools.BaseExtensions.Helper
{
    ...
    public static class EncryptionHelper
    {
        ...
        {
            //可以使用span再次提高下性能
            char[] words = customWords ?? ['O', 'o', '0'];//混淆字符
        ...
        public static string GetNewGuidString(
            GuidFormatEnum foramatEnum = GuidFormatEnum.D,
            bool isUpper = false)
        {
            return isUpper
                ? Guid.NewGuid().ToString(foramatEnum.GetFormatString()).ToUpper()
                : Guid.NewGuid().ToString(foramatEnum.GetFormatString());
        }
