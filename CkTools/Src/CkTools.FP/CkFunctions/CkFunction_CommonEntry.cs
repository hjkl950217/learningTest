...
{
    /// <summary>
    /// 函数式功能-基于Entry提供的处理函数<para></para>
    /// 解决一些批量处理，需要顺序的问题
    /// </summary>
    public static partial class CkFunctions
    {
        ...
        /// </value>
        internal static Func<
            Func<Action<LinkedList<Entry<T?>>>>,
            Action<T?[]>> BatchProcessObject<T>(Func<T?, bool> judge)
        {
            return
                ...
                exps =>
                {
                    ...
                    //判断
                    LinkedList<Entry<T?>> errorList = new();
        ...
        internal static Func<LinkedList<Entry>, string> JoinIndex(string prefix = "exp")
        {
            return arr => string.Join(",", arr.Select(t => $"{prefix}{t.Index}"));
        }

        internal static Func<LinkedList<Entry<T?>>, string> JoinIndex2<T>(string prefix = "exp")
        {
            return arr => string.Join(",", arr.Select(t => $"{prefix}{t.Index}"));
        }
        ...
        /// </summary>
        internal static Action<LinkedList<Entry<T?>>> DefaultBatchThrow<T>()
        {
            return errorArray
        ...
        /// 默认,将条目聚集后抛出一个<see cref="ArgumentNullException"/>
        /// </summary>
        internal static Action<LinkedList<Entry>> DefaultBatchThrow()
        {
            return errorArray
