...
namespace 技术点验证
{
    public static class ZeroWidthCodec
    {
        ...
        {
            // 将字符串拆分成单个字符
            char[] pubMsg = pub.ToCharArray();
            string hiddenMsg = Str2Bin(Encoding.UTF8.GetBytes(hidden));// 将需要隐藏的字符串转换成二进制格式
        ...
        {
            string encodeText = new StringBuilder(string.Join(" ", Encoding.UTF8.GetBytes(str).Select(byt => Convert.ToString(byt, 2).PadLeft(8, '0'))))
                .Replace('\u0030', '\u200B')
                .Replace('\u0031', '\u200C')
                .Replace('\u0020', '\u200D')
                .ToString();
        ...
        public static string Decrypt(string pub)
        {
            ...
            string message = Bin2Str(unwrapped == "false" ? Hidden2Bin(pub) : Hidden2Bin(unwrapped));
